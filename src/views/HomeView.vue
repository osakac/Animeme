...
import { loadNewEpisodes, loadSchedule, loadSliderData } from '@/api/anilibria.api'
...
import type { Anime, ScheduleNow, SliderAnime } from '@/types/anilibria.types'
...
onMounted(async () => {
  const data = await Promise.all([loadSliderData(), loadNewEpisodes(6), loadSchedule('now')])
